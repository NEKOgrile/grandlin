...
export default function DragonBallFloat({ imagePath, shinyImagePath, seed }: DragonBallFloatProps) {
  ...
  // Same depth styles as MagikarpBackground - no transparency
  const getDepthStyles = (depth: number) => {
    const depthMap = {
      1: { zIndex: -40, opacity: 1, scale: 0.5 },
      2: { zIndex: -30, opacity: 1, scale: 0.6 },
      3: { zIndex: -20, opacity: 1, scale: 0.7 },
      4: { zIndex: -10, opacity: 1, scale: 0.8 },
      5: { zIndex: -5, opacity: 1, scale: 0.9 },
    };
